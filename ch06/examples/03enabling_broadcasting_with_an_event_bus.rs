...
async fn consume_event_bus(event_bus: Arc<EventBus<f32>>) {
    ...
    loop {
        ...
        match event {
            Some(event) => {
                println!("id: {}, event: {}", handle.id, event);
                if event == 3.0 {
                    break;
                }
            }
...
async fn main() {
    ...
    let one = tokio::task::spawn(async move {
        consume_event_bus(bus_one).await;
    });
    let two = tokio::task::spawn(async move {
